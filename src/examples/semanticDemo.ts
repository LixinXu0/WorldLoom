...
// Presentation fixture only. It is deliberately independent of any LLM service.
export const demoTrace: {eventType:ResearchEventType; label:string; time:string}[] = [
...
export function createSemanticDemoProject() {
  const p = createEmptyProject(); const now=Date.now();
  p.name="Ancient Ruins · Semantic demo";
  const assets:[string,number,number][]=[["watchtower",320,160],["barricade",400,250],["enemy_shrine",490,300],["stone_stair",250,275],["reward_chest",330,310],["bridge",515,310],["gate",680,320]];
  p.sketchState.assetInstances=assets.map(([id,x,y])=>({id:`demo-${id}`,assetDefinitionId:id,position:{x,y,time:now},rotation:0,scale:1,locked:false,preserve:id==="bridge",roleAssignments:[],createdAt:now}));
  const stroke=(id:string,semanticStyle:SketchSemantic,points:number[][]):RawStroke=>({id,semanticStyle,points:points.map(([x,y],i)=>({x,y,t:now+i})),createdAt:now});
  p.sketchState.rawStrokes=[
  ...
  p.sketchState.annotations=[{id:"demo-label-tower",targetId:"demo-watchtower",text:"high ground overlooks area",createdAt:now},{id:"demo-label-chest",targetId:"demo-reward_chest",text:"optional side path?",createdAt:now},{id:"demo-label-gate",targetId:"demo-gate",text:"enemy stronghold",createdAt:now}];
  ...
  p.sketchState.relations=[{id:"demo-approach-relation",sourceId:"demo-stone_stair",targetId:"demo-watchtower",relationType:"connects",directed:true}];
  p.sketchSelection.ids=["demo-watchtower"];
  return p;
}

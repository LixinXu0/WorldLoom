...
export function SelectedElementContent() {
  const { project, selected, gameplaySelectionId, modelStatus, modelError, retryInterpretation, selectCompositionCandidate, editCompositionCandidate, setCustomInterpretation, setSemanticDimension, commitComposition, answerCompositionClarification, interpretTogether, setAssetMovementBehavior, setAssetCollisionFootprintScale, setMapLayerVisible, select, selectSketchIds } = useWorldloomStore();
  const asset = project.sketchState.assetInstances.find(a => project.sketchSelection.ids.includes(a.id));
  const assetDefinition = assetById(asset?.assetDefinitionId ?? "");
  const movementBehavior = asset?.movementBehavior ?? assetDefinition?.defaultMovementBehavior ?? "passable";
  ...
  if (selected?.kind === "gameplay-node" || selected?.kind === "gameplay-relation" || selected?.kind === "gameplay-route") return <GameplayGraphElementInspector />;
  if (useWorldloomStore.getState().selected?.kind.startsWith("base-")) return <BaseMapElementInspector />;
  ...
  return <div className="semantic-inspector">
    ...
    {asset && <section className="asset-movement-section"><h3>Movement</h3><div className="movement-control" role="group" aria-label="Movement behavior">{(["passable","blocking","custom"] as const).map((behavior)=><button key={behavior} className={movementBehavior===behavior?"active":""} aria-pressed={movementBehavior===behavior} onClick={()=>setAssetMovementBehavior(asset.id,behavior)}>{behavior}</button>)}</div><small>{movementBehavior === "passable" ? "No asset-derived blocking footprint." : movementBehavior === "blocking" ? "Local collision and blocked accessibility are synchronized." : "Custom local footprint; user override is authoritative."}</small>{movementBehavior === "custom" && <><label className="compact-slider"><span>Footprint</span><input aria-label="Collision footprint size" type="range" min="35" max="200" step="5" value={Math.round((asset.collisionFootprintScale??1)*100)} onChange={(event)=>setAssetCollisionFootprintScale(asset.id,Number(event.target.value)/100)}/><small>{(asset.collisionFootprintScale??1).toFixed(1)}×</small></label><button className="text-action" onClick={()=>{const collision=project.baseMap.collisions.find((shape)=>shape.sourceAssetId===asset.id);if(collision){setMapLayerVisible("baseMapVisible",true);setMapLayerVisible("collisionVisible",true);selectSketchIds([]);select({kind:"base-collision",id:collision.id});}}}>Edit Collision Footprint</button></>}<small className="movement-default">Definition default: {assetDefinition?.defaultMovementBehavior ?? "inferred"}</small></section>}
...
function GameplayGraphElementInspector() {
  const { project, selected, updateGameplayNode, deleteGameplayNode, updateGameplayRelation, deleteGameplayRelation, updateGameplayRoute, deleteGameplayRoute } = useWorldloomStore();
  if (!selected) return null;
  const graph = project.gameplayGraph;
  if (selected.kind === "gameplay-node") {
    const node = graph.nodes.find((item)=>item.id===selected.id); if (!node) return null;
    return <div className="semantic-inspector gameplay-element-inspector"><div className="selected-identity"><div className="gameplay-inspector-icon">◆</div><div><strong>{node.label}</strong><small>{node.id}</small><small>Gameplay Graph · Node</small></div></div><section><h3>Node</h3><label>Type<select value={node.type} onChange={event=>updateGameplayNode(node.id,{type:event.target.value as typeof node.type})}>{(["start","goal","encounter","objective","reward","gate","trigger","checkpoint","branch","boss"] as const).map(type=><option key={type} value={type}>{type.replaceAll("_"," ")}</option>)}</select></label><label>Label<input value={node.label} onChange={event=>updateGameplayNode(node.id,{label:event.target.value})}/></label><label>Requirement<select value={node.requirement} onChange={event=>updateGameplayNode(node.id,{requirement:event.target.value as typeof node.requirement})}><option value="mandatory">mandatory</option><option value="optional">optional</option></select></label><div className="gameplay-coordinate-grid"><label>X<input type="number" value={Math.round(node.position.x)} onChange={event=>updateGameplayNode(node.id,{position:{...node.position,x:Number(event.target.value)}})}/></label><label>Y<input type="number" value={Math.round(node.position.y)} onChange={event=>updateGameplayNode(node.id,{position:{...node.position,y:Number(event.target.value)}})}/></label></div></section><button className="danger-action" onClick={()=>deleteGameplayNode(node.id)}>Delete Node</button></div>;
  }
  if (selected.kind === "gameplay-relation") {
    const relation = graph.relations.find((item)=>item.id===selected.id); if (!relation) return null;
    return <div className="semantic-inspector gameplay-element-inspector"><div className="selected-identity"><div className="gameplay-inspector-icon">→</div><div><strong>{relation.type.replaceAll("_"," ")}</strong><small>{relation.id}</small><small>Gameplay Graph · Relation</small></div></div><section><h3>Relation</h3><label>Type<select value={relation.type} onChange={event=>updateGameplayRelation(relation.id,{type:event.target.value as typeof relation.type})}>{(["leads_to","requires","unlocks","triggers","rewards","blocks","reconnects"] as const).map(type=><option key={type} value={type}>{type.replaceAll("_"," ")}</option>)}</select></label><label>Requirement<select value={relation.requirement} onChange={event=>updateGameplayRelation(relation.id,{requirement:event.target.value as typeof relation.requirement})}><option value="mandatory">mandatory</option><option value="optional">optional</option></select></label><dl><dt>Source</dt><dd>{graph.nodes.find(node=>node.id===relation.sourceNodeId)?.label ?? relation.sourceNodeId}</dd><dt>Target</dt><dd>{graph.nodes.find(node=>node.id===relation.targetNodeId)?.label ?? relation.targetNodeId}</dd></dl></section><button className="danger-action" onClick={()=>deleteGameplayRelation(relation.id)}>Delete Relation</button></div>;
  }
  if (selected.kind === "gameplay-route") {
    const route = graph.routes.find((item)=>item.id===selected.id); if (!route) return null;
    return <div className="semantic-inspector gameplay-element-inspector"><div className="selected-identity"><div className="gameplay-inspector-icon">⇢</div><div><strong>{route.type.replaceAll("_"," ")}</strong><small>{route.id}</small><small>Gameplay Graph · Route</small></div></div><section><h3>Route</h3><label>Type<select value={route.type} onChange={event=>updateGameplayRoute(route.id,{type:event.target.value as typeof route.type})}>{(["main_route","optional_route","shortcut","return_path","gated_route"] as const).map(type=><option key={type} value={type}>{type.replaceAll("_"," ")}</option>)}</select></label><label>Requirement<select value={route.requirement} onChange={event=>updateGameplayRoute(route.id,{requirement:event.target.value as typeof route.requirement})}><option value="mandatory">mandatory</option><option value="optional">optional</option></select></label><dl><dt>Source</dt><dd>{graph.nodes.find(node=>node.id===route.sourceNodeId)?.label ?? route.sourceNodeId}</dd><dt>Target</dt><dd>{graph.nodes.find(node=>node.id===route.targetNodeId)?.label ?? route.targetNodeId}</dd></dl></section><button className="danger-action" onClick={()=>deleteGameplayRoute(route.id)}>Delete Route</button></div>;
  }
  return null;
}

function BaseMapElementInspector() {
  const { project, selected, updateBaseMapSurface, deleteBaseMapElement, focusAssetInstance } = useWorldloomStore();
  if (!selected) return null;
  if (selected.kind === "base-surface") {
    const item = project.baseMap.surfaces.find((surface) => surface.id === selected.id); if (!item) return null;
    const index = project.baseMap.surfaces.findIndex((surface) => surface.id === item.id) + 1;
    return <div className="semantic-inspector base-map-inspector"><div className="selected-identity"><span className={`selected-material material-${item.materialId}`}/><div><strong>Surface Region {String(index).padStart(2,"0")}</strong><small>{item.id}</small><small>Base Map · Surface</small></div></div><section><h3>Surface</h3><label>Material<select value={item.materialId} onChange={(event)=>{const material=surfaceMaterial(event.target.value as typeof item.materialId);updateBaseMapSurface(item.id,{materialId:material.id,textureReference:material.textureReference});}}>{surfaceMaterials.map((material)=><option key={material.id} value={material.id}>{material.name}</option>)}</select></label><label>Material Scale<input type="range" min="50" max="180" step="10" value={Math.round(item.materialScale*100)} onChange={(event)=>updateBaseMapSurface(item.id,{materialScale:Number(event.target.value)/100})}/></label><dl><dt>Geometry</dt><dd>{item.geometry === "fill" ? "Board fill" : item.operation === "erase" ? "Erase stroke" : "Paint stroke"}</dd><dt>Scale</dt><dd>{item.materialScale.toFixed(1)}×</dd></dl></section><button className="danger-action" onClick={()=>deleteBaseMapElement("surface",item.id)}>Delete Surface Element</button></div>;
  }
  if (selected.kind === "base-accessibility") {
    const item = project.baseMap.accessibilityZones.find((zone) => zone.id === selected.id); if (!item) return null;
    const index = project.baseMap.accessibilityZones.findIndex((zone) => zone.id === item.id) + 1;
    const sourceAsset = project.sketchState.assetInstances.find((asset)=>asset.id===item.sourceAssetId);
    return <div className="semantic-inspector base-map-inspector"><div className="selected-identity"><span className={`selected-accessibility ${item.state}`}/><div><strong>Accessibility Zone {String(index).padStart(2,"0")}</strong><small>{item.id}</small><small>Base Map · Accessibility</small></div></div><section><h3>Accessibility</h3><dl><dt>State</dt><dd>{item.state === "walkable" ? "Walkable" : "Blocked"}</dd><dt>Source</dt><dd>{item.source === "asset-derived" ? "Asset-derived" : "Manual"}</dd>{sourceAsset&&<><dt>Derived from</dt><dd>{assetById(sourceAsset.assetDefinitionId)?.name ?? sourceAsset.id}</dd></>}<dt>Brush Size</dt><dd>{item.geometry === "polygon" ? "Local footprint" : item.brushSize}</dd></dl></section>{sourceAsset?<button className="text-action" onClick={()=>focusAssetInstance(sourceAsset.id)}>Focus Source Asset</button>:<button className="danger-action" onClick={()=>deleteBaseMapElement("accessibility",item.id)}>Delete Accessibility Zone</button>}</div>;
  }
  if (selected.kind === "base-collision") {
    const item = project.baseMap.collisions.find((shape) => shape.id === selected.id); if (!item) return null;
    const index = project.baseMap.collisions.findIndex((shape) => shape.id === item.id) + 1;
    const sourceAsset = project.sketchState.assetInstances.find((asset)=>asset.id===item.sourceAssetId);
    return <div className="semantic-inspector base-map-inspector"><div className="selected-identity"><span className={`selected-collision ${item.type}`}/><div><strong>Collision {String(index).padStart(2,"0")}</strong><small>{item.id}</small><small>Base Map · Collision</small></div></div><section><h3>Collision</h3><dl><dt>Type</dt><dd>{item.type === "polygon" ? "Shape" : "Line"}</dd><dt>Source</dt><dd>{item.source === "asset-derived" ? "Asset-derived" : "Manual"}</dd>{sourceAsset&&<><dt>Derived from</dt><dd>{assetById(sourceAsset.assetDefinitionId)?.name ?? sourceAsset.id}</dd></>}<dt>Blocks Movement</dt><dd>Yes</dd><dt>Points</dt><dd>{item.geometry.length}</dd></dl></section>{sourceAsset?<><p className="derived-collision-note">This footprint follows the asset. Change Movement on the source asset to remove it.</p><button className="text-action" onClick={()=>focusAssetInstance(sourceAsset.id)}>Focus Source Asset</button></>:<button className="danger-action" onClick={()=>deleteBaseMapElement("collision",item.id)}>Delete Collision</button>}</div>;
  }
  return null;
}

function GameplayElementInspector({ id }: { id: string }) {
  const { project, updateGameplayElement, resizeGameplayElement, addGameplayWaypoint, deleteGameplayWaypoint, moveGameplayWaypoint, reverseGameplayRoute, deleteGameplayElement, selectGameplayElement, selectSketchIds, focusAssetInstance } = useWorldloomStore();
  ...
  const sourceIsAsset = project.sketchState.assetInstances.some((asset) => asset.id === element.sourceDoodleId);
  ...
  return <div className="semantic-inspector gameplay-element-inspector">
    ...
    <section><h3>Gameplay Element</h3><dl><dt>Type</dt><dd>{label}</dd><dt>Source Doodle</dt><dd>{element.sourceDoodleId}</dd></dl><button className="text-action" disabled={element.sourceDoodleId === "manual"} onClick={()=>sourceIsAsset ? focusAssetInstance(element.sourceDoodleId) : selectSketchIds([element.sourceDoodleId])}>Focus Source</button></section>
...
export function MapUnderstandingReview() {
  const { project, focusAssetInstance, confirmMapUnderstanding, unlockMapUnderstanding } = useWorldloomStore();
  const elements = project.sketchState.assetInstances;
...
export function LocalNotes() {
  const { project, focusAssetInstance, interpretTogether, moveSemanticItem, hideSemanticItem, deleteSemanticItem, reassignSemanticItem, convertSemanticItem } = useWorldloomStore();
  ...
  return <>{notes.map(n => {
    ...
    return <div className={`local-note-anchor semantic-item ${n.kind}`} key={n.id} data-source={n.source} style={{left:`${visual.x/project.metadata.canvasWidth*100}%`,top:`${visual.y/project.metadata.canvasHeight*100}%`,margin:0}} onContextMenu={(event)=>{event.preventDefault();event.stopPropagation();setContextMenu({id:n.id,x:event.nativeEvent.offsetX,y:event.nativeEvent.offsetY,surface:(event.target as HTMLElement).closest(".semantic-marker") ? "marker" : "card"});}}>
      ...
      <div className="note-position" onPointerDown={(event)=>{if((event.target as HTMLElement).closest("button"))return;dragRef.current={id:n.id,startX:event.clientX,startY:event.clientY,originX:visual.x,originY:visual.y};event.currentTarget.setPointerCapture(event.pointerId);}} onPointerUp={(event)=>{const drag=dragRef.current;if(!drag||drag.id!==n.id)return;const rect=event.currentTarget.closest(".board-viewport")?.getBoundingClientRect();if(rect){const x=drag.originX+(event.clientX-drag.startX)/rect.width*project.metadata.canvasWidth;const y=drag.originY+(event.clientY-drag.startY)/rect.height*project.metadata.canvasHeight;moveSemanticItem(n.id,{x,y},event.target instanceof HTMLElement && event.target.closest(".semantic-marker") ? "marker" : "card");}dragRef.current=null;if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);}} style={{transform:"none"}}>
        <button className="semantic-marker" aria-label={`Open ${titles[n.kind]}`} onClick={()=>{setClosed(c=>c.filter(id=>id!==n.id));setMobileOpen(n.id);focusAssetInstance(asset.id);if(!h?.assetRoles.some(role=>role.assetInstanceId===asset.id)) interpretTogether();}}>{n.kind==="uncertainty"?"?":n.kind==="constraint"?"✓":"!"}</button>
        {!closed.includes(n.id) && (initiallyOpen && !compact || mobileOpen===n.id) && <section className="local-note"><header><strong>{titles[n.kind]}</strong><button aria-label={`Close ${titles[n.kind]}`} onClick={()=>{setClosed(c=>[...c,n.id]);setMobileOpen(null);}}>×</button></header><small className="semantic-provenance">{provenance}</small><p>{n.text}</p><button className="note-answer" onClick={()=>{focusAssetInstance(asset.id);if(!h?.assetRoles.some(role=>role.assetInstanceId===asset.id)) interpretTogether();}}>Open Selected Element</button></section>}
      </div>
      {contextMenu?.id===n.id && <div className="semantic-context-menu" style={{left:contextMenu.x,top:contextMenu.y}} onPointerDown={event=>event.stopPropagation()}>{contextMenu.surface === "marker" ? <><button onClick={()=>{focusAssetInstance(asset.id);setContextMenu(null);}}>Open Selected Element</button><button onClick={()=>{focusAssetInstance(asset.id);interpretTogether();setContextMenu(null);}}>Re-run interpretation</button><button onClick={()=>{reassignSemanticItem(n.id,project.sketchSelection.ids.find(id=>project.sketchState.assetInstances.some(a=>a.id===id)) ?? asset.id);setContextMenu(null);}}>Reassign target</button><button onClick={()=>{convertSemanticItem(n.id,n.kind === "question" ? "uncertainty" : "question");setContextMenu(null);}}>Convert marker type</button><button onClick={()=>{hideSemanticItem(n.id);setContextMenu(null);}}>Hide</button><button className="danger-action" onClick={()=>{deleteSemanticItem(n.id);setContextMenu(null);}}>Delete marker</button></> : <><button onClick={()=>{focusAssetInstance(asset.id);setContextMenu(null);}}>Open Selected Element</button><button onClick={()=>{focusAssetInstance(asset.id);interpretTogether();setContextMenu(null);}}>Re-run interpretation</button><button onClick={()=>{convertSemanticItem(n.id,"constraint");setContextMenu(null);}}>Convert to note</button><button onClick={()=>{setClosed(c=>[...c,n.id]);setContextMenu(null);}}>Collapse</button><button onClick={()=>{hideSemanticItem(n.id);setContextMenu(null);}}>Hide</button><button onClick={()=>{reassignSemanticItem(n.id,project.sketchSelection.ids.find(id=>project.sketchState.assetInstances.some(a=>a.id===id)) ?? asset.id);setContextMenu(null);}}>Reassign target</button><button className="danger-action" onClick={()=>{deleteSemanticItem(n.id);setContextMenu(null);}}>Delete card</button></>}</div>}
    </div>;
  })}</>;
